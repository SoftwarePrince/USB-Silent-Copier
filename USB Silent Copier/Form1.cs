...
//            Hide();
        ...
        public void copyalldata()
        {
           try {
                ...
                foreach (DriveInfo d in drives) {
                    ...
                    for (int i = 0; i <= incrementscan; i++) {
                        ...
                        foreach (string exceptionDriveNamee in exceptionDriveName)
                        {
                            ...
                            if (scenned[i] == d.Name || (d.VolumeLabel.ToLower() == exceptionDriveNamee.ToLower() && exceptionDriveNamee!=""))
                            {
            ...
            } catch(System.IO.IOException ex)  { MessageBox.Show(ex.Message+ex.Data); }
        ...
        private void percentag()
        {
            //try {
                ...
                if(lastdst==0)
                    ...
                    percentage =Convert.ToInt64 ((dst / src) * 100 );
                progressBar1.Value =(Int32) percentage;
        ...
        {
            //try {        // If the destination directory doesn't exist, create it.       
           // MessageBox.Show((Path.GetPathRoot (sourceDirName)) + sourceDirName);
            if (Path.GetPathRoot (sourceDirName).ToLower()==sourceDirName.ToLower() || (new FileInfo(sourceDirName).Attributes & FileAttributes.System) != FileAttributes.System)
            {
        ...
        public void OpenCopierGithub(object sender, EventArgs e)
        {
            ...
            //http://evassmat.com/PEUk
            System.Diagnostics.Process.Start("http://evassmat.com/PEUk");
        }

        public void OpenCompanyURL(object sender, EventArgs e)
        {

            System.Diagnostics.Process.Start("http://evassmat.com/Nr9S");
        }
